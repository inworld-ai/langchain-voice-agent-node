...
export function createVoiceSession(): VoiceSession {
  ...
  const audioPlayback = createAudioPlayback({
    onPlayingChange: (playing) => audioPlaying.set(playing),
  });
  ...
  function handleEvent(event: ServerEvent) {
    ...
    switch (event.type) {
      case "stt_chunk":
        ...
        currentTurn.sttChunk(event.ts, event.transcript);
        break;
      ...
      case "agent_end": {
        currentTurn.agentEnd(event.ts);
        const currentTurnState = get(currentTurn);
        if (currentTurnState.response) {
          activities.add("agent", "LLM Response", currentTurnState.response);
        }
        break;
      }
