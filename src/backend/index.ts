...
if (!existsSync(STATIC_DIR)) {
  console.error(
    `Web build not found at ${STATIC_DIR}.\n` +
      "Run 'npm run build:frontend' from the project root."
  );
  process.exit(1);
}
 ...
 *
 * This function takes a stream of upstream voice agent events and processes them.
 * When agent_chunk events arrive, it sends the text to Inworld for TTS synthesis.
 * Audio is streamed back as tts_chunk events as it's generated.
 * All upstream events are passed through unchanged.
 *
 * It uses a producer-consumer pattern where:
 * - Producer: Reads events from eventStream, passes them through, and sends agent text to Inworld
 * - Consumer: Receives audio chunks from Inworld and yields them as tts_chunk events
 *
 * @param eventStream - An async iterator of upstream voice agent events
 * @param existingTts - Optional pre-created TTS instance (for warmup)
...
async function* ttsStream(
  ...
  /**
   * Promise that reads events from the upstream stream and sends text to Inworld.
   *
   * This runs concurrently with the consumer, continuously reading events
   * from the upstream stream and forwarding agent text to Inworld for synthesis.
  ...
  const producer = iife(async () => {
    try {
      ...
      // Signal to Inworld that text sending is complete
      await tts.close();
    }
  });

  /**
   * Promise that receives audio events from Inworld.
   *
...
app.get(
  ...
  upgradeWebSocket(async () => {
    ...
    const outputEventStream = ttsStream(agentEventStream, tts);
...
console.log(`Chat with the voice agent at http://localhost:${PORT}`);
